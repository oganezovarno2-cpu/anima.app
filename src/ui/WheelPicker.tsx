...
export default function WheelPicker({
  ...
  onChange:(v:string|number)=>void, visible?: number
}) {
  const ref=React.useRef<HTMLDivElement>(null)
  React.useEffect(()=>{
    const i=Math.max(0, values.findIndex(v=>String(v)===String(value)))
    const el=ref.current; if(!el) return
    const row=36; el.scrollTo({ top: i*row - Math.floor(visible/2)*row, behavior:"smooth" })
  ...
  return (
    <div className="relative mx-auto w-[260px]">
      <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 h-10 rounded-xl border border-white/60 bg-white/40 shadow"/>
      <div ref={ref} onScroll={onScroll}
        className="h-48 overflow-y-scroll no-scrollbar py-20 text-center">
        {values.map((v,i)=>(
